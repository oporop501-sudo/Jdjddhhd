...
# Проверяем, где запущен бот: на Render всегда есть переменная PORT
    ...
    def run_fake_server():
        ...
        try:
            with socketserver.TCPServer(("", port), handler) as httpd:
                print(f"[RENDER] Фейковый сервер запущен на порту {port}")
                httpd.serve_forever()
        except Exception as e:
            print(f"[RENDER] Ошибка веб-сервера: {e}")

    threading.Thread(target=run_fake_server, daemon=True).start()
else:
    print("[PYDROID 3] Запуск на телефоне. Веб-сервер отключен.")

# Твой точный токен без опечаток
